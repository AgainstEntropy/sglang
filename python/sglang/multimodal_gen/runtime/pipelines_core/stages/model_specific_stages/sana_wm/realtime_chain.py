...
"""SANA-WM realtime stage CHAIN — LingBot-pattern decomposition of the former
mega-stage (``SanaWMRealtimeStage.forward``).

Per tick the pipeline runs:
...
Each stage owns a small ``BaseRealtimeState`` blob. The chain stages subclass the
helper host ``SanaWMRealtimeStage`` so the parity-validated bodies (cond-frame
fp32 encode, camera single-sourcing, refiner runner build, conv-cache decode)
are inherited verbatim rather than copied.
"""
...
class SanaWMRealtimeLatentPrepStage(SanaWMRealtimeStage):
    ...
    uniform chunks from the seeded fallback generator. The plan contains more
    than one chunk only when the refiner block grid lags the stage-1 chunk grid
    (the option-b boundary tick) so every tick emits frames."""
...
class SanaWMChunkedRefinerChainStage(SanaWMRealtimeStage):
    ...
    def _build_chain_refiner_runner(
        ...
        # Reuse the parity-validated builder via a state adapter (it reads
        # prompt/sink/block/kv_max from the legacy state shape).
